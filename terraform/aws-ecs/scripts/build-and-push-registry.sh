...
ECR_REPO_NAME="mcp-gateway-registry"
...
docker build -f docker/Dockerfile.registry -t "${ECR_REPO_NAME}:latest" "$SCRIPT_DIR" || exit 1
...
echo "   registry_image_uri = \"${ECR_REPO_URI}:latest\""
